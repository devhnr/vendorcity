                                ...
                                @for ($k = 0; $k < count($formFields); $k++)
                                    ...
                                    @if ($result1[$i]->lable_name == $formFields[$k]->lable_name)
                                        ...
                                        @if ($result1[$i]->type == '4')
                                            ...
                                            @endforeach
                                            <p class="form-error-text"
                                                id="formfield_value_1{{ $formFields[$k]->id }}"
                                                style="color: red; margin-top: 10px;">
                                            </p>
                                        @endif
                                        @if ($result1[$i]->type == '5')
                            ...
                            @for ($i = 0; $i < count($result2); $i++)
                                @for ($k = 0; $k < count($formFields); $k++)
                                    ...
                                    @if ($result2[$i]->lable_name == $formFields[$k]->lable_name)
                                        ...
                                        @if ($result2[$i]->type == '4')
                                            <br>
                                            <input name="form_field_checkbox_id2[]" type="hidden" class="m-0"
                                                id="form_field_id[]" value="{{ $formFields[$k]->id }}">
                                            <label
                                                class="form-label fw500 dark-color">{{ $formFields[$k]->lable_name }}</label>
                                            @foreach ($form_additionalData as $form_additional)
                ...
                @if ($result1[$i]->lable_name == $formFields[$k]->lable_name)
                    ...
                    @if ($result1[$i]->type == '4' && $result1[$i]->is_active == '1')
                        ...
                        if (checkboxes.length === 0) {
                            var errorMessage =
                                "Please select at least one option for {{ $formFields[$k]->lable_name }}";
                            $('#formfield_value_1{{ $formFields[$k]->id }}').html(errorMessage);
                            $('#formfield_value_1{{ $formFields[$k]->id }}').show().delay(0).fadeIn('show');
                            $('#formfield_value_1{{ $formFields[$k]->id }}').show().delay(2000).fadeOut(
                                'show');
                            $('html, body').animate({
                                scrollTop: $('#formfield_value_1{{ $formFields[$k]->id }}').offset()
                                    .top - 150
                            }, 1000);
                            return false;
                        }
            ...
            @for ($k = 0; $k < count($formFields); $k++)
                @if ($result2[$i]->lable_name == $formFields[$k]->lable_name)
                    ...
                    @if ($result2[$i]->type == '4' && $result2[$i]->is_active == '1')
                        var checkbo = document.querySelectorAll(
                            'input[name^="formfield_checkbox_{{ $formFields[$k]->id }}"]:checked');
                        ...
                        if (checkbo.length === 0) {
                            var errorMessage =
                                "Please select at least one option for {{ $formFields[$k]->lable_name }}";
                            $('#formfield_value_2{{ $formFields[$k]->id }}').html(errorMessage);
                            $('#formfield_value_2{{ $formFields[$k]->id }}').show().delay(0).fadeIn('show');
                            $('#formfield_value_2{{ $formFields[$k]->id }}').show().delay(2000).fadeOut(
                                'show');
                            $('html, body').animate({
                                scrollTop: $('#formfield_value_2{{ $formFields[$k]->id }}').offset()
                                    .top - 150
                            }, 1000);
                            return false;
                        }
                    ...
                    // @if ($result2[$i]->type == '4' && $result2[$i]->is_active == '1')
                    //     var checkbox = jQuery("#formfield_value_check{{ $formFields[$k]->id }}").val();
                    //     // alert(drop_down);
                    //     if (checkbox == '') {
                    //         jQuery('#checkbox2_error').html("Please Enter {{ $formFields[$k]->lable_name }}");
                    //         jQuery('#checkbox2_error').show().delay(0).fadeIn('show');
                    //         jQuery('#checkbox2_error').show().delay(2000).fadeOut('show');
                    //         $('html, body').animate({
                    //             scrollTop: $('#formfield_value_check{{ $formFields[$k]->id }}').offset().top -
                    //                 150
                    //         }, 1000);
                    //         return false;
                    //     }
