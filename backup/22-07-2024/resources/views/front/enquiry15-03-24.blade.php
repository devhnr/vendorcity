                                        ...
                                        @if ($result2[$i]->type == '4')
                                            ...
                                            @foreach ($form_additionalData as $form_additional)
                                                <input name="formfield_checkbox_{{ $formFields[$k]->id }}[]"
                                                    ...
                                                    placeholder="" value="{{ $form_additional->form_option }}"
                                                    id="formfield_value_check{{ $formFields[$k]->id }}">

                                                <label>{{ $form_additional->form_option }}</label>
                                            @endforeach
                                            <p class="form-error-text" id="checkbox2_error"
                                                style="color: red; margin-top: 10px;">
                                            </p>
                                        @endif
                                        {{-- @if ($result2[$i]->type == '5')
            ...
            @for ($k = 0; $k < count($formFields); $k++)
                @if ($result1[$i]->lable_name == $formFields[$k]->lable_name)
                    ...
                    @if ($result1[$i]->type == '4' && $result1[$i]->is_active == '1')
                        ...
                        // alert(checkbox);
                        if (checkbox == '') {
                            jQuery('#formfield_value_checkbox{{ $formFields[$k]->id }}').html(
                                "Please Enter {{ $formFields[$k]->lable_name }}");
                            jQuery('#formfield_value_checkbox{{ $formFields[$k]->id }}').show().delay(0).fadeIn(
                                'show');
                            jQuery('#formfield_value_checkbox{{ $formFields[$k]->id }}').show().delay(2000).fadeOut(
                                'show');
                            $('html, body').animate({
                                scrollTop: $('#formfield_value_checkbox{{ $formFields[$k]->id }}').offset()
                                    .top - 150
                            }, 1000);
                            return false;
                        }
